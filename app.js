...
let delTarget = null, zeroTarget = null, editTarget = null;
...
function handleWS(msg){
  ...
  switch(type){
    ...
    case 'SHOP_COMPLETED':
      // 在庫の更新をローカルに反映してからリフレッシュ
      if(payload.updatedInv && payload.updatedInv.length){
        payload.updatedInv.forEach(updated=>{
          const idx=invList.findIndex(i=>i.master_id===updated.master_id);
          if(idx>=0) invList[idx]={...invList[idx],...updated};
          else invList.push(updated);
        });
    ...
    case 'SHOP_EDITED': {
      const idx = shopList.findIndex(s=>s.id===payload.id);
      if(idx>=0){
        shopList[idx] = {...shopList[idx], ...payload};
        saveCache(CK.shop,shopList); renderList();
      }
      break;
    }
    case 'INV_EDITED': {
      const idx = invList.findIndex(i=>i.id===payload.id);
      if(idx>=0){
        invList[idx] = {...invList[idx], ...payload};
        saveCache(CK.inv,invList); renderInv();
      }
      break;
    }
...
function populateCatSelects(){
  ...
  ['fCat','ifCat','editCat'].forEach(id=>{ const el=document.getElementById(id); if(el) el.innerHTML=opts; });
}
...
function shopItemHTML(item){
  ...
  return `<div class="shop-item${item.checked?' checked':''}">
    ...
    <div style="display:flex;flex-direction:column;gap:4px">
      <button class="btn-x" style="font-size:13px;color:var(--text2)" onclick="openEdit('shop',${item.id})">✎</button>
          ...
          <div class="inv-item">
            <div class="inv-header">
              ...
              <div style="display:flex;gap:4px;align-items:center">
                <button class="btn-x" style="font-size:14px;color:var(--text2)" onclick="openEdit('inv',${iv.id})">✎</button>
...
async function setStage(id,stage){
  ...
  try{
    ...
    // 「少ない」または「なし」に変わったときにリスト追加を確認
    if((stage==='few'||stage==='none') && prev!=='few' && prev!=='none'){
      zeroTarget=id;
      const msg = stage==='none'
        ? `「<strong>${iv.name}</strong>」の在庫がなくなりました。<br>買い物リストに追加しますか？`
        : `「<strong>${iv.name}</strong>」の在庫が少なくなりました。<br>買い物リストに追加しますか？`;
      document.getElementById('ov-zero-body').innerHTML=msg;
      openOv('ov-zero');
    }
...
async function confirmZero(){
  ...
  if(iv){
    try{
      // 在庫には残したままリストだけ追加（zero-to-listではなくadd-to-listを使う）
      const result=await api(`/api/inventory/${iv.id}/add-to-list`,'POST',{added_by:me});
      showToast(result.added?'買い物リストに追加しました':'既にリストにあります');
      // リストにWSでSHOP_ADDEDが来るのでrenderListは不要
    } catch(e){ showToast('処理に失敗しました。再度お試しください。'); }
  }
...
function openEdit(type, id){
  editTarget = {type, id};
  let item = null;
  if(type==='shop') item = shopList.find(x=>x.id===id);
  if(type==='inv')  item = invList.find(x=>x.id===id);
  if(!item) return;

  document.getElementById('ov-edit-title').textContent = type==='shop'?'リスト商品を編集':'在庫商品を編集';
  document.getElementById('editName').value  = item.name  || '';
  document.getElementById('editBrand').value = item.brand || '';
  document.getElementById('editStore').value = item.store || '';
  document.getElementById('editNote').value  = item.note  || '';
  // カテゴリー選択肢を更新してから値をセット
  populateCatSelects();
  setTimeout(()=>{ const el=document.getElementById('editCat'); if(el) el.value=item.category_id||''; },50);
  openOv('ov-edit');
}
...
async function confirmEdit(){
  ...
  try {
    const path = type==='shop' ? `/api/items/${id}/edit` : `/api/inventory/${id}/edit`;
    const updated = await api(path,'PATCH',{name,brand,store,note,category_id});
    if(type==='shop'){
      const idx=shopList.findIndex(s=>s.id===id);
      if(idx>=0){ shopList[idx]={...shopList[idx],...updated}; saveCache(CK.shop,shopList); renderList(); }
    } else {
      const idx=invList.findIndex(i=>i.id===id);
      if(idx>=0){ invList[idx]={...invList[idx],...updated}; saveCache(CK.inv,invList); renderInv(); }
    }
    // マスターも更新
    const m=await api('/api/history/masters'); masters=m; saveCache(CK.masters,m);
    closeOv('ov-edit');
