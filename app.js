  ...
  document.getElementById('mealResults').innerHTML=meals.map((m,i)=>`
    <div class="meal-card">
      ...
      <div class="meal-bd" id="mb-${i}">
        ${m.ingredients_stock?.length?`<div class="meal-sec">🥦 在庫から使う食材</div><div class="ing-row">${m.ingredients_stock.map(s=>typeof s==='object'?`<span class="ing ing-s">${s.name}<small style="opacity:.7;margin-left:3px">${s.amount||''}</small></span>`:`<span class="ing ing-s">${s}</span>`).join('')}</div>`:''}
        ${m.ingredients_buy?.length?`<div class="meal-sec">🛒 購入が必要な食材</div><div class="ing-row">${m.ingredients_buy.map(s=>typeof s==='object'?`<span class="ing ing-b">${s.name}<small style="opacity:.7;margin-left:3px">${s.amount||''}</small></span>`:`<span class="ing ing-b">${s}</span>`).join('')}</div>`:''}`
