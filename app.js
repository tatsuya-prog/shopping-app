...
async function initSW(){
  ...
  try {
    ...
    navigator.serviceWorker.addEventListener('controllerchange', () => {
      showToast('新しいバージョンがあります。↺ボタンで更新してください。');
    });
...
async function subscribePush(reg){
  ...
  try {
    ...
    pushSub = sub;
    await fetch(API+'/api/push/subscribe',{ method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({subscription:sub, user:me}) });
...
async function refreshAll(){
  try {
    ...
    renderAll();
  } catch(e){ showToast('オフライン - キャッシュを表示中'); }
}
...
function handleWS(msg){
  ...
  switch(type){
    ...
    case 'SHOP_COMPLETED':
      ...
      break;
    case 'INV_ADDED':
      if(!invList.find(i=>i.id===payload.id)){ invList.push(payload); saveCache(CK.inv,invList); renderInv(); }
      break;
    case 'INV_UPDATED': {
...
document.addEventListener('click', e=>{
  if(!e.target.closest('.suggest-wrap')){
    document.getElementById('sgList').style.display = 'none';
    const sg2 = document.getElementById('ifSgList');
    if(sg2) sg2.style.display = 'none';
  }
});
  ...
  wrap.innerHTML = invList.map(iv=>`
    <div class="inv-item">
      <div class="inv-header">
        ...
        <button class="btn-x" onclick="askDel('inv',${iv.id})" style="font-size:16px;padding:4px 8px">✕</button>
      </div>
      <div class="stage-row">
...
async function setStage(id, stage){
  ...
  const prev = iv.stage;
  iv.stage = stage; saveCache(CK.inv,invList); renderInv();
  try {
      ...
      <div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--rs);overflow:hidden;margin-bottom:12px">
        ${d.top.map((r,i)=>`
          <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 12px;border-bottom:${i<d.top.length-1?'1px solid var(--border)':'none'}">
            <div><span style="font-size:13px;font-weight:700;color:var(--text2);margin-right:8px">${i+1}</span>
            <span style="font-size:14px;font-weight:500">${r.name}</span></div>
      ...
      <div style="display:flex;gap:8px;margin-bottom:12px">
        ${d.byWho.map(r=>{
          ...
          return `<div style="flex:1;background:var(--surface);border:1px solid var(--border);border-radius:var(--rs);padding:12px;text-align:center">
            ...
            <div style="font-size:22px;font-weight:700">${r.count}</div>
...
async function generateMeals(){
  ...
  document.getElementById('mealResults').innerHTML =
    '<div class="ai-loading"><div class="spinner"></div>献立を考えています...</div>';
  ...
  try {
    const data = await api('/api/meal/suggest','POST',{days:selDays, moods, source:selSrc, inventory, adults, kids});
    renderMeals(data.meals || [], data.tokenInfo);
  } catch(e){
    document.getElementById('mealResults').innerHTML =
      '<div class="ai-loading">⚠️ 取得に失敗しました。もう一度お試しください。</div>';
  }
  btn.disabled = false;
}
...
function renderMeals(meals, tokenInfo){
  ...
  // 料金情報を表示
  const tokenEl = document.getElementById('mealTokenInfo');
  if(tokenEl && tokenInfo){
    tokenEl.textContent = `入力:${tokenInfo.inputTokens}トークン / 出力:${tokenInfo.outputTokens}トークン / 約¥${tokenInfo.costJpy}`;
    tokenEl.style.display = 'block';
  } else if(tokenEl){
    tokenEl.style.display = 'none';
  }
  document.getElementById('mealResults').innerHTML = meals.map((m,i)=>{
    // Geminiがオブジェクトで返す場合も文字列で返す場合も対応
    const ingList = (arr,cls) => (arr||[]).map(s=>{
      const text = typeof s === 'object' ? (s.name||s.item||s.ingredient||JSON.stringify(s)) : String(s);
      const amount = typeof s === 'object' && (s.amount||s.quantity) ? ` (${s.amount||s.quantity})` : '';
      return `<span class="ing ${cls}">${text}${amount}</span>`;
    }).join('');
      ...
      <div class="meal-hd" onclick="toggleMeal(${i})">
        <div style="flex:1;min-width:0">
          ...
          <div class="meal-tags">
            ...
            ${m.people_note?`<span class="meal-tag meal-tag-note">⚠️ ${m.people_note}</span>`:''}
          </div>
        </div>
        <div class="meal-toggle" id="mt-${i}">▾</div>
      </div>
      <div class="meal-bd" id="mb-${i}">
        ${m.ingredients_stock&&m.ingredients_stock.length?`<div class="meal-sec">🥦 在庫から使う食材</div><div class="ing-row">${ingList(m.ingredients_stock,'ing-s')}</div>`:''}
        ${m.ingredients_buy&&m.ingredients_buy.length?`<div class="meal-sec">🛒 購入が必要な食材</div><div class="ing-row">${ingList(m.ingredients_buy,'ing-b')}</div>`:''}
...
async function confirmDel(){
  ...
  try {
    if(type==='shop'){
      ...
      shopList=shopList.filter(s=>s.id!==id); saveCache(CK.shop,shopList); renderList(); updateBadges();
    } else if(type==='inv'){
      await api(`/api/inventory/${id}`,'DELETE');
      invList=invList.filter(i=>i.id!==id); saveCache(CK.inv,invList); renderInv();
    } else if(type==='rec'){
      await api(`/api/recurring/${id}`,'DELETE');
      recurring=recurring.filter(r=>r.id!==id); saveCache(CK.rec,recurring); renderRec();
    }
  } catch(e){ showToast('削除に失敗しました'); }
...
function initPullToRefresh(){
  ...
  const bar = document.getElementById('pullIndicator');
  const threshold = 75;
  ...
  document.addEventListener('touchend', e=>{
    ...
    bar.style.opacity=0; bar.style.transform='scaleX(0)';
    if(dist>threshold && window.scrollY===0){
      refreshAll().then(()=>showToast('更新しました'));
    }
    pulling=false;
  },{passive:true});
}
