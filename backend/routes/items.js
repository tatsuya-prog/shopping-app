...
router.post('/complete', async (req, res) => {
  ...
  try {
    ...
    await client.query('COMMIT');
    // 在庫の更新情報をブロードキャスト（満タンになった商品を同期）
    const { rows: updatedInv } = await pool.query(
      `SELECT i.id, i.master_id, i.stage, m.name, m.brand, m.store, m.category_id,
              c.name AS category_name, c.color AS category_color
       FROM inventory i JOIN masters m ON i.master_id=m.id
       LEFT JOIN categories c ON m.category_id=c.id
       WHERE i.master_id = ANY($1::int[])`,
      [rows.filter(r=>r.freq==='once').map(r=>r.master_id)]
    );
    broadcast('SHOP_COMPLETED', { bought_by, date: today, count: rows.length, updatedInv });
...
router.patch('/:id/edit', async (req, res) => {
  ...
  try {
    await client.query('BEGIN');
    // まず対象のshop_listを取得
    const { rows: sl } = await client.query('SELECT master_id FROM shop_list WHERE id=$1', [req.params.id]);
    if (!sl.length) { await client.query('ROLLBACK'); return res.status(404).json({ error: '見つかりません' }); }
    const masterId = sl[0].master_id;
    // mastersを更新
    await client.query(
    ...
    const cat = catRows[0] || null;
    const updated = { id: +req.params.id, master_id: masterId, name, brand: brand||'', note: note||'', store: store||'', category_id: category_id||null, category_name: cat?cat.name:null, category_color: cat?cat.color:null };
    broadcast('SHOP_EDITED', updated);
    res.json(updated);
  } catch(e) {
    await client.query('ROLLBACK');
    res.status(500).json({ error: e.message });
