...
const { pool, touchSyncState } = require('../db');
...
router.get('/', async (req, res) => {
  try {
    const { rows } = await pool.query(`
      SELECT s.id, s.master_id, s.checked, s.added_by, s.freq,
             m.name, m.brand, m.note, m.store
      FROM shop_list s
...
router.post('/', async (req, res) => {
  const { name, brand, note, store, added_by, freq } = req.body;
  ...
  try {
    ...
    await touchSyncState(client);
    await client.query('COMMIT');

    const item = { id: ir.rows[0].id, master_id: masterId, checked: false, added_by, freq: itemFreq, name, brand: brand||'', note: note||'', store: store||'' };
    // WSは最小ペイロード
