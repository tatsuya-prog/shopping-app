  ...
  for (const model of models.slice(0, 5)) {
    try {
      ...
      try {
        ...
        // Gemini 2.5 Flash 料金: 入力$0.30/1M, 出力$2.50/1M
        const costUsd = (inputTokens * 0.0000003) + (outputTokens * 0.0000025);
        const costJpy = costUsd * 150; // 1USD=150円
        const tokenInfo = { inputTokens, outputTokens, costUsd: +costUsd.toFixed(6), costJpy: +costJpy.toFixed(4) };
        console.log(`✅ Gemini success: ${model} | tokens: in=${inputTokens} out=${outputTokens} cost=¥${costJpy.toFixed(4)}`);
        return res.json({ ...parsed, tokenInfo });
      } catch(parseErr) {
        console.log('JSON parse error:', parseErr.message);
        // JSONが途中で切れている場合、取得できた meals 配列を救済
        try {
          const partialMatch = jsonMatch[0].match(/"meals"\s*:\s*(\[[\s\S]*)/);
          if (partialMatch) {
            // 不完全な配列を修復：最後の完全なオブジェクトまでを取得
            const arrStr = partialMatch[1];
            const completeObjects = [];
            let depth = 0, inStr = false, start = 0;
            for (let ci = 0; ci < arrStr.length; ci++) {
              const ch = arrStr[ci];
              if (ch === '"' && arrStr[ci-1] !== '\\') inStr = !inStr;
              if (!inStr) {
                if (ch === '{') { if (depth === 1) start = ci; depth++; }
                if (ch === '}') { depth--; if (depth === 1) completeObjects.push(arrStr.slice(start, ci+1)); }
                if (ch === '[') depth++;
                if (ch === ']') depth--;
              }
            }
            if (completeObjects.length > 0) {
              const meals = completeObjects.map(o => JSON.parse(o));
              console.log(`✅ Gemini partial success: ${model} (${meals.length} meals)`);
