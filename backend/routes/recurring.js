...
async function runCheck(client) {
  ...
  for (const rec of rows) {
    const ex = await client.query('SELECT id FROM shop_list WHERE master_id=$1 AND checked=false',[rec.master_id]);
    if (!ex.rows.length) {
      await client.query('INSERT INTO shop_list(master_id,added_by,freq) VALUES($1,$2,$3)',[rec.master_id,rec.added_by,rec.freq]);
      added++;
    }
