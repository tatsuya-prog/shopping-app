...
app.use('/api/history/masters', (req, res, next) => {
  res.set('Cache-Control', 'public, max-age=60'); // 1分キャッシュ
  next();
});
...
setNotifyHook(async (type, payload) => {
  if (type === 'SHOP_ADDED') {
    pool.query('INSERT INTO push_queue(title,body) VALUES($1,$2)',
...
async function sendQueuedNotifications() {
  try {
    const { rows } = await pool.query('SELECT * FROM push_queue ORDER BY created_at LIMIT 20');
