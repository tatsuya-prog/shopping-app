  ...
  switch(type) {
    case 'SHOP_ADDED':
      if (!shopList.find(s=>s.id===payload.id)) {
        ...
        // zero-to-listで追加した場合はtoastを出さない（confirmZeroで出す）
        if (!zeroTarget) showToast(`${payload.added_by}が「${payload.name}」を追加`);
      }
      break;
    case 'SHOP_CHECKED': {
...
async function generateMeals() {
  ...
  try {
    const data=await api('/api/meal/suggest','POST',{days:selDays,moods,source:selSrcMode,inventory,adults,children});
...
function renderMeals(meals) {
  ...
  document.getElementById('mealResults').innerHTML=meals.map((m,i)=>{
    const ingHtml = (arr, cls) => (arr||[]).map(s=>
      typeof s==='object'
        ? `<span class="ing ${cls}">${s.name}${s.amount?`<small style="opacity:.7;margin-left:3px">${s.amount}</small>`:''}</span>`
        : `<span class="ing ${cls}">${s}</span>`
    ).join('');
    return `<div class="meal-card">
      <div class="meal-hd" onclick="toggleMeal(${i})">
        <div>
          ...
          <div class="meal-meta">
            ${m.mood?`<span class="meal-mood">${m.mood}</span>`:''}
            ${m.calories_per_person?`<span class="meal-cal">🔥 ${m.calories_per_person}</span>`:''}
          </div>
          ${m.people_note?`<div style="font-size:11px;color:var(--warn);margin-top:3px">⚠️ ${m.people_note}</div>`:''}
        </div>
        <div class="meal-toggle" id="mt-${i}">▾</div>
      </div>
      <div class="meal-bd" id="mb-${i}">
        ${m.ingredients_stock?.length?`<div class="meal-sec">🥦 在庫から使う食材</div><div class="ing-row">${ingHtml(m.ingredients_stock,'ing-s')}</div>`:''}
        ${m.ingredients_buy?.length?`<div class="meal-sec">🛒 購入が必要な食材</div><div class="ing-row">${ingHtml(m.ingredients_buy,'ing-b')}</div>`:''}
...
(function() {
  ...
  document.addEventListener('touchend', e => {
    ...
    const bar = document.getElementById('pullBar');
    if (bar) { bar.style.opacity = 0; bar.style.transform = 'scaleX(0)'; }
