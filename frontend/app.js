...
function renderHist() {
  ...
  wrap.innerHTML=histData.slice(0,60).map(h=>{
    ...
    return `<div class="hist-item">
      <div><div class="hist-name">${h.name}</div><div class="hist-sub">${h.brand||''}${h.store?` ／ ${h.store}`:''}</div></div>
      <div><div class="hist-date">${h.bought_at}</div><span class="hist-who tag ${cls}">${h.bought_by}</span></div>
    </div>`;
  }).join('');
}
